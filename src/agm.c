...
int _process_mp4(const char *input_file, const char *output_file, int output_width, int output_height, const char *palette_file, const char *noDither_method, const char *dither_method, int lookback, bool has_transparent_color, const uint8_t transparent_rgb[3]) {
    ...
    // Create a Palette object from the palette file.
    Palette palette;
    if (_load_gimp_palette(palette_file, &palette) != 0) {
        fprintf(stderr, "Could not load palette file: %s\n", palette_file);
        avcodec_free_context(&codec_ctx);
        avformat_close_input(&format_ctx);
        return -1;
    }
        ...
        if (packet.stream_index == (int)video_stream_index) {
            if (avcodec_send_packet(codec_ctx, &packet) == 0) {
                while (avcodec_receive_frame(codec_ctx, frame) == 0) {
                    ...
                    uint8_t *newNo = malloc(frame_pixel_count * sizeof(uint8_t));
                    uint8_t *newDither = malloc(frame_pixel_count * sizeof(uint8_t));
                    if (newNo == NULL || newDither == NULL) {
                        exit(1);
                    }
                    ...
                    if (first_frame) {
                        ...
                        dither_lookback(oldNo, newNo, oldFinal, newDither,
