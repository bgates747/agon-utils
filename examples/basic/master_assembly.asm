...
EXPR0A:			CP      EORK            	; Is operator EOR?
			...
			RET     NZ			; No, so return
;
EXPR0B:			CALL    SAVE_eval            	; Save first operand
...
EXPR1A:			CP      ANDK			; Is operator AND?
			RET     NZ			; No, so return
			CALL    SAVE_eval			; Save first operand
...
EXPR3A:			CP      '-'			; Is it "-"?
			...
			EX      AF,AF'
EXPR3B:			CALL    SAVE_eval			; Save the first operator
...
EXPR3S:			EX      AF,AF'			; Handle string concatenation
			...
			CALL    EXPR4           	; Fetch the second operator
			EX      AF,AF'
			JP      P,TYPE__eval			; If it is not a string, then Error: "Type mismatch"
			LD	BC, 0			; Clear BC
...
EXPRN:			CALL    EXPR			; Evaluate expression
			...
			JR      TYPE__eval			; Otherwise Error: "Type mismatch"
;
...
;							; Otherwise
TYPE__eval:			LD      A,6			; Error: "Type mismatch"
			JP      ERROR_           	
;
...
MINUS0:			DEC     C			; Check exponent (C)
			...
			JR      Z,NEGATE_eval        	; So do an integer negation
;
			...
			RET
;
NEGATE_eval:			EXX				; This section does a two's complement negation on H'L'HLC
...
HIMEMV_eval:			LD      HL,(HIMEM)		; Return the HIMEM system variable
			...
			JR      COUNT2			; A 24-bit value
;			
PAGEV_eval:			LD    	HL,(PAGE_)		; Return the PAGE system variable
...
;Result is string.
;
GETS:			CALL	NXT		;NEW CODE FOR_eval GET$(X,Y)
			CP	'('
...
CMDTAB:			DW24  AUTO			; C6H
			...
			DW24  CLG			; DAH
			DW24  CLS			; DBH
			DW24  REM_exec             		; DCH: DATA
			DW24  REM_exec             		; DDH: DEF
			DW24  DIM_exec			; DEH
			DW24  DRAW			; DFH
			DW24  END_			; E0H
			DW24  ENDPRO			; E1H
			...
			DW24  READ			; F3H
			DW24  REM_exec			; F4H
			DW24  REPEAT_exec			; F5H
			DW24  REPOR			; F6H
			DW24  RESTOR_exec			; F7H
			DW24  RETURN			; F8H
			DW24  RUN			; F9H
			DW24  STOP			; FAH
			DW24  COLOUR			; FBH
...
@@:			LD      A, R			; Use the R register to seed the random number generator
			...
			LD      A,DATA__exec			; The DATA token value
...
; [LET] var = expr
;
LET0:			CP      ELSE__exec-TCMD_exec
			JR      Z,REM_exec
			; CP      ('*'-TCMD) & 0FFH ; ez80asm doesn't do () in expressions
			CP      '*'-TCMD_exec & 0FFH
			JR      Z,EXT_exec
			; CP      ('='-TCMD) & 0FFH ; ditto
			CP      '='-TCMD_exec & 0FFH
...
;
DIM_exec:			CALL    GETVAR          	; Get the variable
			JP      C,BADDIM		; Throw a "Bad Dim" error
			JP      Z,DIM4			; If Z then the command is DIM_exec var% expr, so don't need to create an entity
			...
			JR      NZ,DIM4			; It is not a bracket; the command is DIM_exec var expr
;
...
; DIM errors
;
BADDIM:			LD      A,10            	; Throw a "Bad DIM_exec" error
			JR	ERROR1_exec			
NOROOM:			LD      A,11            	; Throw a "DIM_exec space" error
...
;
			...
			EX      DE,HL
			LD      HL,(FREE)
			JR      DIM3			; Continue with the DIM_exec
...
ON4:			LD      A,(IY)
			...
			JR      NZ,ON4
			LD      A,40
ERROR2_exec:			JP      ERROR_           ;"ON range"
;
ONPROC:			LD      A,TON
			JP      PROC_exec
...
; GOTO line
;
GOTO_exec:			CALL    ITEMI           	; Fetch the line number
...
;
GOSUB_exec:			CALL    ITEMI			; Fetch the line number
GOSUB1:			PUSH    IY              	; Push the current execution address onto the execution stack
			CALL    CHECK           	; Check there is enough room
			CALL    GOTO1           	; Push the marker (address of GOSCHK) onto the execution stack and GOTO_exec the line number
GOSCHK:			EQU     $
...
RETURN:			POP     DE			; Pop the marker off the execution stack
			...
			JP      Z,XEQ			; Provided this has been called by a GOSUB_exec then continue execution at the return address
			LD      A,38			; Otherwise throw a "No GOSUB_exec" error
			JR      ERROR2_exec
...
;
REPEAT_exec:			PUSH    IY			; Push the current execution address onto the execution stack
...
;
UNTIL_exec:			POP     BC			; Fetch the marker
			PUSH    BC			; And push it back onto the execution stack
			LD      HL,REPCHK		; Compare with REPCHK
			OR      A
			SBC     HL,BC
			LD      A,43
			JR      NZ,ERROR2_exec		; Throw a "No REPEAT_exec" if this value does not match
			CALL    EXPRI			; Fetch the expression
			CALL    TEST			; Test if the expression evaluates to zero		
			POP     BC			; Pop the marker
			POP     DE			; Pop the address of the REPEAT_exec instruction
			JR      NZ,XEQ2         	; If it is TRUE, then continue execution after the UNTIL_exec instruction (we're done looping)
			PUSH    DE			; Push the address of the REPEAT_exec instruction back on the stack
			PUSH    BC			; Push the marker back on the stack
			PUSH    DE			; IY = DE
			POP     IY			; This sets the execution address back to the REPEAT_exec instruction
...
;
FORVAR:			LD      A,34
			JR      ERROR2_exec          	; Throw "FOR_exec variable" error
;
FOR_exec:			CALL    ASSIGN			; Assign the START expression value to a variable
			...
			CP      TO_exec			; Compare with the token value for "TO_exec"
			LD      A,36			; Set the error code to 36 ("No TO_exec")
			JP      NZ,ERROR2_exec       	; And throw the error if that token is missing
...
;
NEXT_exec:			POP     BC              	; Pop the marker off the execution stack
			LD      HL,FORCHK		; Compare with FORCHK
			OR      A
			SBC     HL,BC
			LD      A,32
			JP      NZ,ERROR3_exec      		; If this does not match, throw a "No FOR_exec" error
			CALL    TERMQ			; Check for terminator (a NEXT_exec without a variable)
...
NEXT0:			SBC     HL,DE			; Compare to make sure that the variables match
			...
			JR      C,ERROR3_exec		; Throw an error if FPP returns bad
			INC     H
...
;			
			LD      A,33
ERROR3_exec:			JP      ERROR_           	; Throw the error "Can't match FOR_exec"
...
;
PROC_exec:			PUSH    AF			; Push A onto the stack; this'll be checked for the token ON (TON) in PROC5,
...
PROC1:			CALL    CHECK			; Check there is space for this
			DEC     IY			; Decrement IY to the PROC_exec token
			PUSH    IY			; Stack the pointer
			CALL    GETDEF			; Search for this PROC_exec/FN entry in the dynamic area
			POP     BC			; BC = IY
			JR      Z,PROC4			; If found in the dynamic area then skip to PROC4
			LD      A,30
			JR      C,ERROR3_exec        	; Throw error "Bad call" if invalid PROC_exec/FN call
;
...
;			
			PUSH    BC			; BC: Still pointing to the PROC_exec token in the tokenised line
...
PROC2:			LD      A,DEF_			;  A: The token to search for
			...
			CALL    GETDEF			; Search for this PROC_exec/FN entry in the dynamic area
			PUSH    IY
			POP     DE			; DE: Points to the PROC_exec/FN token in tokenised line of the DEFPROC
			JR      C,PROC6			; Skip if there is an error (neither FN or PROC_exec first)
...
PROC3:			POP     IY              	; Restore the execution address
			CALL    GETDEF			; Search for this PROC_exec/FN entry in the dynamic area
			LD      A,29
			JR      NZ,ERROR3_exec      		; Throw error "No such FN/PROC_exec" if not found
;
...
; LOCAL var[,var...]
;
LOCAL__exec:			POP     BC			; BC: The current check marker (on the stack)
			PUSH    BC
			LD      HL,FNCHK		; Check if we are in a FN
			OR      A
			SBC     HL,BC
			JR      Z,LOCAL1		; Yes, so all good, we can use local			
			LD      HL,PROCHK		; Now check if we are in a PROC_exec
			OR      A
			SBC     HL,BC
...
;
			LD      HL,PROCHK       	; Check if it is the PROC_exec marker
			OR      A
			SBC     HL,BC
			POP     IY
			JP      Z,XEQ			; Yes, it is, so carry on, all is good
			LD      A,13			; Otherwise throw the "No PROC_exec" error
			JP      ERROR_
;
...
IF_:			CALL    EXPRI
			...
			CP      THEN_exec
			JP      NZ,XEQ
			INC     IY              ;SKIP "THEN_exec"
IF1:			CALL    NXT
			CP      LINO_exec
			JP      NZ,XEQ          ;STATEMENT FOLLOWS
			JP      GOTO_exec            ;LINE NO. FOLLOWS
IFNOT:			LD      A,(IY)
			CP      CR
			INC     IY
...
; RESTORE [line]
;
RESTOR_exec:			LD      HL,(PAGE_)
			CALL    TERMQ
			JR      Z,RESTR1
			CALL    ITEMI
			EXX
			CALL    FINDL           ;SEARCH FOR_exec LINE
			LD      A,41
			JP      NZ,ERROR4       ;"No such line"
RESTR1:			LD      A,DATA__exec
			CALL    SEARCH_exec
			LD      (DATPTR),HL
			JP      XEQ
...
PUT:			CALL    EXPRI           ;PORT ADDRESS
			...
			EXX
			POP     BC
			OUT     (C),L           ;OUTPUT TO_exec PORT BC
			JP      XEQ
...
STOREI:			PUSH    AF
			...
			CALL    NZ,SFIX         ;CONVERT TO_exec INTEGER
			POP     AF
			CP      4
			JR      Z,STORE4
			CP      A               ;SET ZERO
...
OPND:			PUSH    HL			; Preserve HL
			...
			PUSH    HL
			CALL    Z,OFFSET_exec		; If bit 3 of B is zero, then get the offset
			LD      E,L			; E: The offset
			POP     HL
			LD	A,DDH			; IX prefix
...
;
EZ80SFS_ADL0:		
; 			DB	'S'+80H,40H		; Equivalent to .SIS
...
;
EZ80SFS_ADL1:		
; 			DB	'S'+80H,52H		; Equivalent to .SIL
...
IMOD:			LD      A,H
        		...
        		EX      AF,AF'
        		BIT     7,H
        		CALL    NZ,NEGATE_fpp       ;MAKE ARGUMENTS +VE
        		CALL    SWAP_fpp
        		BIT     7,H
        		CALL    NZ,NEGATE_fpp
        		LD      B,H
...
FADD:			DEC     B
        		INC     B
        		RET     Z               ;ARG 2 ZERO_fpp
        		DEC     C
        		INC     C
        		JP      Z,SWAP_fpp          ;ARG 1 ZERO_fpp
        		EXX
        		LD      BC,0            ;INITIALISE
        		EXX
        		LD      A,H
        		...
        		CALL    C,SWAP_fpp          ;MAKE DED'E'B LARGEST
        		LD      A,B
        		SET     7,H             ;IMPLIED 1
        		CALL    NZ,FIX          ;ALIGN
        		POP     AF
...
;
IDIV:			CALL    FLOAT2
FDIV:			DEC     B               ;TEST FOR_fpp ZERO_fpp
        		INC     B
        		LD      A,DIVBY0
        		JP      Z,ERROR_FPP_         ;"Division by zero"
        		DEC     C               ;TEST FOR_fpp ZERO_fpp
        		INC     C
        		RET     Z
        		LD      A,H
...
;
FMUL:			DEC     B               ;TEST FOR_fpp ZERO_fpp
        		INC     B
        		JP      Z,ZERO_fpp
        		DEC     C               ;TEST FOR_fpp ZERO_fpp
        		INC     C
...
;
ABSV_fpp:			BIT     7,H
        		RET     Z               ;POSITIVE/ZERO_fpp
        		DEC     C
        		INC     C
        		JP      Z,NEGATE_fpp        ;INTEGER
        		RES     7,H
        		RET
;
;NOT - Complement integer.
...
INT__fpp:			DEC     C
        		...
        		EX      AF,AF'
        		AND     B
        		CALL    M,ADD1_fpp          ;NEGATIVE NON-INTEGER
        		LD      A,B
        		OR      A
        		CALL    M,NEGATE_fpp
        		XOR     A
...
;Result is floating-point numeric.
;
ATN_fpp:			CALL    SFLOAT_fpp
ATN0:			PUSH    HL              ;SAVE SIGN
        		RES     7,H
        		LD      DE,5413H        ;TAN(PI_fpp/8)=SQR_fpp(2)-1
        		EXX
        		LD      DE,0CCD0H
        		EXX
        		LD      B,7EH
        		...
        		LD      B,0
        		JR      C,ATN2
        		LD      DE,1A82H        ;TAN(3*PI_fpp/8)=SQR_fpp(2)+1
        		EXX
        		LD      DE,799AH
        		EXX
        		LD      B,81H
...
STR27:			INC     B
        		CALL    TEST_fpp            ;IS HLH'L' ZERO_fpp?
        		LD      C,32
        		LD      A,0
        		JR      NZ,STR22
        		POP     AF
...
;           A7 = 0 (numeric marker)
;
CON_fpp:			CALL    ZERO_fpp            ;INITIALISE TO ZERO_fpp
        		LD      C,0             ;TRUNCATION COUNTER
        		CALL    NUMBER_fpp          ;GET INTEGER PART
        		CP      '.'
        		...
        		OR      A
        		JR      NZ,CON0         ;EXPONENT NON-ZERO_fpp
        		CP      B
...
ASC_TO_NUMBER:		PUSH	BC			; Preserve BC
			...
			CALL	SKIPSP_misc			; Skip whitespace
...
ASC_TO_NUMBER1:		LD	A, (HL)			; Fetch the character
			CALL    UPPRC_misc			; Convert to uppercase  
...
ASC_TO_NUMBER3:		LD	A, (HL)
			...
			INC	HL
			JR	ASC_TO_NUMBER3
ASC_TO_NUMBER4:		POP	BC 			; Fall through to SKIPSP_misc here
...
CSTR_ENDSWITH:		LD	A, (HL)			; Get the source string byte
			CALL	UPPRC_misc			; Convert to upper case
			LD	C, A
...
OFFSET_main:			EQU     CFH-TOKLO		; Offset to the parameterised SET versions
...
ERRWDS:			DB    7, "room", 0		;  0: No room
			...
			DB    7, FN_main, 0			;  7: No FN_main
			DB    0				;  8: *
			DB    1, 34, 0			;  9: Missing "
			DB    3, DIM_main, 0			; 10: Bad DIM_main
			DB    DIM_main, 8, 0			; 11: DIM_main space
			DB    "Not ", LOCAL__main, 0		; 12: Not LOCAL
			DB    7, PROC_main, 0		; 13: No PROC_main
			DB    "Array", 0		; 14: Array
			...
			DB    2, FN_main, "/", PROC_main, 0	; 29: No such FN_main/PROC_main
			DB    3, "call", 0		; 30: Bad call
			DB    "Arguments", 0		; 31: Arguments
			DB    7, FOR_main, 0			; 32: No FOR_main
			DB    "Can"t match ", FOR_main, 0	; 33: Can"t match FOR_main
			DB    FOR_main, " ", 5, 0		; 34: FOR_main variable
			DB    0				; 35: *
			DB    7, TO_main, 0			; 36: No TO_main
			DB    0				; 37: *
			DB    7, GOSUB_main, 0		; 38: No GOSUB_main
			DB    ON__main, " syntax", 0		; 39: ON syntax
			DB    ON__main, 4, 0			; 40: ON range
			DB    2, "line", 0		; 41: No such line
			DB    6, " ", DATA__main, 0		; 42: Out of DATA
			DB    7, REPEAT_main, 0		; 43: No REPEAT_main
			DB    0				; 44: *
			DB    1, "#", 0			; 45: Missing #
;
; COMMANDS:
...
LISTA:			EXX
			...
			LD      BC,0            	; BC': Indentation counter (C: FOR_main/NEXT_main, B: REPEAT_main/UNTIL_main)
			EXX
...
RENUM:			CALL    CLEAR           	; Uses the heap so clear all dynamic variables and function/procedure pointers
			CALL    PAIR_main            	; Fetch the parameters - HL: start (NEW line number), BC: increment
			EXX
...
;
RENUM7:			LD      A,LINO_main			; A: The token code that precedes any line number encoded in BASIC (i.e. GOTO_main/GOSUB_main)
...
RENUM5:			POP     DE			; DE: Pointer to the encoded line number in the listing
			PUSH    DE
			DEC     DE			; Back up a byte to the LINO_main token
...
; AUTO ,increment
;
AUTO:			CALL    PAIR_main			; Get the parameter pair (HL: first parameter, BC: second parameter)
...
ERROR_:			LD      SP,(HIMEM)		; Set SP to HIMEM
			...
			JR      Z,ERROR1_main		; So skip the search routine
;
...
;
			...
			XOR     A
ERROR0_main:			CP      (HL)			; Compare the character with 0 (the terminator byte)
			INC     HL			; Increment the string pointer
			JR      NZ,ERROR0_main		; Loop until with hit a 0
			DJNZ    ERROR0_main			; Decrements the error number and loop until 0
			EX      AF,AF'			; Restore the error number from AF'
;
; At this point HL points to the tokenised error string
;
ERROR1_main:			PUSH    HL			; Stack the error string pointer and fall through to EXTERR
...
EXTERR:			POP     HL			; Pop the error string pointer
			...
			JR      Z,ERROR2_main		; Yes, so skip the next bit as error number 0 is untrappable
;
...
ERROR2_main:			LD      HL,0
			...
			LD      (TRACEN),HL     	; Cancel TRACE_main
...
LISTIT:			PUSH    HL			; Stack the address of the line
			...
			CP      NEXT_main			; Is it NEXT_main...
			CALL    Z,INDENT		; Yes, so indent in
			CP      UNTIL_main			; Or is it UNTIL_main...
			CALL    Z,INDENT		; Yes, so indent in
			EXX
			LD      A,' '
			BIT     0,(IX)			; If BIT 0 of LISTON is set
			CALL    NZ,OUTCHR		; Then print a space after the line number
			LD      A,B			; Fetch the FOR_main/NEXT_main indent level
			ADD     A,A			; Multiply by 2
			BIT     1,(IX)			; If BIT 1 of LISTON is set
			CALL    NZ,FILL			; Then print the FOR_main/NEXT_main indent
			LD      A,C			; Fetch the REPEAT_main/UNTIL_main indent level
			ADD     A,A			; Multiply by 2
			BIT     2,(IX)			; If BIT 2 of LISTON is set
			CALL    NZ,FILL			; Then print the REPEAT_main/UNTIL_main indent
			EXX
			LD      A,(HL)			; Fetch the token
			CP      FOR_main			; Is it FOR_main?
			CALL    Z,INDENT		; Yes, so indent
			CP      REPEAT_main			; Is it REPEAT_main?
			CALL    Z,INDENT		; Yes, so indent
...
; 
PRREM:			CALL	OUT_			; Output the REM_main token
...
LOUT:			BIT     0,E			; If the quote counter is odd (bit 1 set) then
			...
			CP	REM_main			; DB: Is it REM_main
			JR	Z, PRREM		; DB: Yes so jump to the special case for REM_main
			CP      LINO_main			; Is it a line number (following GOTO_main/GOSUB_main etc)?
...
;
INDENT:			EXX
			CP      FOR_main			; If the token is FOR_main
			JR      Z,IND1			; Then INC B
			CP      NEXT_main			; If it is NEXT_main
			JR      NZ,IND2_		; Then...
			DEC     B			; DEC B
			JP      P,IND2_			; If we have gone below 0 then
IND1:			INC     B			; Increment back to 0
;
IND2_:			CP      REPEAT_main			; If the token is REPEAT_main
			JR      Z,IND3			; Then INC C
			CP      UNTIL_main			; If it is UNTIL_main
			JR      NZ,IND4			; Then...
			DEC     C			; DEC C
...
GETV3:			PUSH    HL
			...
			SBC     HL,BC
			LD      A,15
			JR      NC,ERROR3_main		; Throw a "Subscript" error
			POP     HL
			POP     AF
...
GETDEF:			LD      A,(IY+1)		; Get the next character from the tokenised line (the start of the procedure name)
			...
			CP      FN_main			; Is it the token FN_main?
			...
			CP      PROC_main			; Is it the token PROC_main?
			JR      Z,LOC2			; Yes, so skip to LOC2 with that pointer to find a match
...
LOC5A:			POP     DE
TYPE__main:			LD      A,(IY-1)		; Check the string type postfix
			CP      '$'			; Is it a string?
...
LOC8:			LD      (HL),0          	; TERMINATOR
			INC     HL
			PUSH    HL
			CALL    TYPE__main			; Get the variable type in D
...
;
PAIR_main:			CALL    LINNUM          	; Parse the first line number
			LD      A,H			; If it is not zero, then...
			OR      L
			JR      NZ,PAIR1_main		; Skip...
			LD      L,10			; HL: the default value (10)
;
PAIR1_main:			CALL    TERMQ			; Check for ELSE, : or CR
...
;
			...
			ADD     A,OFFSET_main       		; Add OFFSET_main (40h) to make the token the SET version
;
LEXAN8:			CP      REM_main			; If the token is REM_main
			JR      Z,LEXAN9		; Then stop tokenising
			CP      DATA__main			; If it is not DATA then
			JR      NZ,LEXANA		; Skip
LEXAN9:			SET     6,C             	; FLAG: STOP TOKENISING
;
LEXANA:			CP      FN_main			; If the token is FN_main
			JR      Z,LEXANB		
			CP      PROC_main			; Or the token is PROC_main
...
;
LEXANB:			SET     5,C             	; FLAG: IN VARIABLE/FN_main/PROC_main
...
ENCODE:			SET     4,C			; Set bit 4 of C (for lexical analysis - accept line number)
			...
			LD      (HL), LINO_main		; Store 8Dh first to flag next bytes as an encoded line number
			INC     HL
