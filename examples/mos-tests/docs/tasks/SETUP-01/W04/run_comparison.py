"""W04 isolated hostfs bootstrap comparison; run with agon-utils .venv."""
...
def observe(variant):
    ...
    destination = PROFILE / "sdcard/mystuff/w04-test.bin"
    ...
    try:
        ...
        assert sha(binary) == sha(destination)
        autoexec.write_bytes(b"SET KEYBOARD 1\r\ncd /mystuff\r\nload w04-test.bin\r\nrun\r\n")
        master, slave = pty.openpty()
        env = os.environ.copy()
        env.update(SDL_VIDEODRIVER="dummy", SDL_AUDIODRIVER="dummy")
        proc = subprocess.Popen(["./fab-agon-emulator", "--renderer", "sw", "-d", "-b", "0"],
        ...
        result = dict(variant=variant, passed=True, backend="hostfs-bootstrap", binary_sha256=sha(binary),
