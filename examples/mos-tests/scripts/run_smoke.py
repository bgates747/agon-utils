"""Maintained smoke observer promoted from SETUP-01 W06; see fixtures/smoke/README.md."""
from pathlib import Path
import hashlib, json, os, pty, re, select, shutil, subprocess, time
PROJECT = Path(__file__).resolve().parents[1]
FIXTURES = PROJECT / "fixtures/smoke"
...
def observe(variant):
    ...
    destination = evidence / "sd.img"
    assert not destination.exists(), "Refusing to overwrite existing W04 deployment"
    raw = bytearray()
    proc = None
    master = None
    started = time.monotonic()
    try:
        subprocess.run([str(PROJECT.parents[1]/".venv/bin/python"), str(PROJECT/"scripts/prepare_sd_image.py"), "--binary", str(binary), "--output", str(destination)], check=True)
        shutil.copy2(destination.with_suffix(".img.json"), evidence/"image-before.json")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Qualified C++/assembly raw-image smoke comparison (not the full MOS suite).")
    parser.add_argument("--output", type=Path, required=True, help="Fresh evidence directory; refuses existing paths")
    args = parser.parse_args()
    RUN = args.output.resolve()
    RUN.mkdir(parents=True, exist_ok=False)
    identities = {name: {"path": str((PROFILE/name).resolve()), "sha256": sha(PROFILE/name)}
        for name in ("fab-agon-emulator", "fab-agon-emulator.bin", "firmware/mos_platform.bin", "firmware/mos_platform.map", "firmware/vdp_platform.so")}
    (RUN/"environment.json").write_text(json.dumps({"backend": "headless-raw-image", "identities": identities}, indent=2)+"\n")
    try:
        results = [observe(v) for v in ("cpp", "assembly")]
        assert results[0]["output_hex"] == results[1]["output_hex"]
        summary = "ALL SELECTED SMOKE CHECKS PASSED — 2 variants\nBackend: headless emulator, raw SD image. Hardware untested.\n"
        (RUN/"summary.txt").write_text(summary)
        print(summary, end="")
    except Exception as error:
        summary = "INCOMPLETE / FAILED SMOKE CHECK — " + str(error) + "\nSee retained per-variant evidence; this is not a full-suite verdict.\n"
        (RUN/"summary.txt").write_text(summary)
        raise
