...
def compare_frames(prev_frame, curr_frame):
    ...
    Compare two byte arrays (of equal length). For each pixel in the current frame:
      ...
      - Otherwise, output 0xF3 (magenta, full alpha).
    Returns a new bytes object.
    """
    return bytes((curr if curr != prev else 0xF3) for prev, curr in zip(prev_frame, curr_frame))
...
def main():
    ...
    output_csv = os.path.join("tests", f"compare_compressions_movie_{target_frame_rate:02d}_diffed.csv")
    ...
    for count, idx in enumerate(selected_indices, start=1):
        ...
        delta_frame = compare_frames(current_frame, prev_frame)
